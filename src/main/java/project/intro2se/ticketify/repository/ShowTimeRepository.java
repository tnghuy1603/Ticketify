...
public interface ShowTimeRepository extends JpaRepository<ShowTime, Long> {
    ...
    @Query("SELECT st FROM ShowTime st JOIN st.room r WHERE r.theater.id = :theateId AND DATE(st.startAt) =: date")
    List<ShowTime> findByTheaterAndDay(@Param("theaterId") Long theaterId, LocalDate date);
    ...
    @Query("SELECT st FROM ShowTime st JOIN st.room r WHERE r.theater.id = :theateId")
